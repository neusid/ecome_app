...
function randomDate(maxDaysAgo = 90) {
  const now = Date.now();
  const past = now - maxDaysAgo * 24 * 60 * 60 * 1000;
...
function generateOrder() {
  ...
  const products = Array.from({ length: numItems }, () => ({
...
async function seed() {
  ...
  for (const o of orders) {
    ...
    byUser[o.id_user] = (byUser[o.id_user] || 0) + 1;
  }

  console.log(`Berhasil menambahkan ${orders.length} orders!`);
